<template>
  <div
    class="design-vertical-guide"
    :class="{ active: active, 'label-always-visible': labelAlwaysVisible }"
    :style="style"
    @mousedown="onMouseDown"
  >
    <div class="design-vertical-guide-indicator"></div>
    <div class="design-vertical-guide-line" :style="lineStyle"></div>
    <div class="design-vertical-guide-label">
...
export default {
  ...
  computed: {
    style() {
      return {
        left: `${this.position * (this.zoom() / 100) - 3}px`,
      };
    },
    lineStyle() {
      return {
  ...
  methods: {
    ...
    onMouseMove(event) {
      if (this.active && event.target.classList.contains("design-surface-overlay")) {
        ...
        const position = (x - 3) * (100 / this.zoom());
        if (this.snapToGrid()) {
